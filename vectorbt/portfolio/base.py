...
class Portfolio(Wrapping, StatsBuilderMixin, PlotsBuilderMixin, metaclass=MetaPortfolio):
    ...
    def from_signals(
        ...
        * `signal_func_nb` and `signal_args`: Custom signal function that returns direction-aware signals.
        ...
        # Perform the simulation
        can_use_static = (
            not signal_func_mode
            and adjust_sl_func_nb is nb.no_adjust_sl_func_nb
            and adjust_tp_func_nb is nb.no_adjust_tp_func_nb
        )

        if can_use_static:
            if ls_mode:
                order_records, log_records = dispatch.simulate_from_signals(
                    target_shape_2d,
                    cs_group_lens,
                    init_cash,
                    call_seq,
                    entries=np.asarray(False),
                    exits=np.asarray(False),
                    direction=np.asarray(Direction.LongOnly),
                    long_entries=broadcasted_args["entries"],
                    long_exits=broadcasted_args["exits"],
                    short_entries=broadcasted_args["short_entries"],
                    short_exits=broadcasted_args["short_exits"],
                ...
                order_records, log_records = dispatch.simulate_from_signals(
                    ...
                    direction=broadcasted_args["direction"],
