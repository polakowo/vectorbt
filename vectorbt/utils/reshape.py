...
from vectorbt.utils import checks, indexes, common
    ...
    if along_axis == 0:
        if checks.is_pandas(arg):
            return arg.vbt.wrap_array(
                ...
                index=indexes.repeat(arg.index, n))
        ...
        if checks.is_pandas(arg):
            return arg.vbt.wrap_array(
                ...
                columns=indexes.repeat(arg.columns, n))
    ...
    if along_axis == 0:
        if arg.ndim == 1:
            if checks.is_pandas(arg):
                return arg.vbt.wrap_array(
                    ...
                    index=indexes.tile(arg.index, n))
        ...
        if checks.is_pandas(arg):
            return arg.vbt.wrap_array(
                ...
                columns=indexes.tile(arg.columns, n))
        ...
        if isinstance(index_from, int):
                    ...
                    if new_index is None:
                        ...
                        if len(index) != len(new_index):
                            ...
                            if ignore_single:
                                ...
                                if len(index) > len(new_index):
                                    new_index = indexes.repeat(new_index, len(index))
                                elif len(index) < len(new_index):
                                    index = indexes.repeat(index, len(new_index))
                        new_index = indexes.stack(new_index, index)
                        if drop_duplicates:
                            new_index = indexes.drop_duplicate_levels(new_index, keep=keep)
            if maxlen > len(new_index):
                ...
                if maxlen > 1 and len(new_index) > 1:
                    raise ValueError("Indexes could not be broadcast together")
                new_index = indexes.repeat(new_index, maxlen)
        else:
        ...
        if checks.is_pandas(old_arg):
            if new_index is None:
                ...
                if old_arg.shape[0] == new_arg.shape[0]:
                    ...
                    new_index = indexes.repeat(old_arg.index, new_arg.shape[0])
            if new_columns is None:
                ...
                if new_arg.ndim == 2:
                    ...
                    if old_arg.shape[1] == new_arg.shape[1]:
                        ...
                        new_columns = indexes.repeat(old_arg.columns, new_arg.shape[1])
        else:
...
def unstack_to_array(arg, levels=None):
    ...
    for i in range(len(levels)):
        vals = indexes.select_levels(sr.index, levels[i]).to_numpy()
...
def unstack_to_df(arg, index_levels=None, column_levels=None, symmetric=False):
    ...
    new_index = np.unique(indexes.select_levels(arg.index, index_levels))
    new_columns = np.unique(indexes.select_levels(arg.index, column_levels))
