...
from vectorbt.utils.combine import *
from vectorbt.utils.common import *
from vectorbt.utils.indexes import *
from vectorbt.utils.indexing import *
from vectorbt.utils.reshape import *
...
class Base_Accessor():
    ...
    def to_1d_array(self):
        return reshape.to_1d(self._obj, raw=True)

    def to_2d_array(self):
        return reshape.to_2d(self._obj, raw=True)
    ...
    def tile(self, n, as_columns=None):
        tiled = reshape.tile(self._obj, n, along_axis=1)
        if as_columns is not None:
            new_columns = indexes.combine(as_columns, reshape.to_2d(self._obj).columns)
            return self.wrap_array(tiled.values, columns=new_columns)
        return tiled

    def repeat(self, n, as_columns=None):
        repeated = reshape.repeat(self._obj, n, along_axis=1)
        if as_columns is not None:
            new_columns = indexes.combine(reshape.to_2d(self._obj).columns, as_columns)
            return self.wrap_array(repeated.values, columns=new_columns)
        return repeated

    def align_to(self, other):
        checks.assert_type(other, (pd.Series, pd.DataFrame))
        obj = reshape.to_2d(self._obj)
        other = reshape.to_2d(other)
    ...
    def broadcast(self_or_cls, *others, **kwargs):
        ...
        if isinstance(self_or_cls, type):
            return reshape.broadcast(*others, **kwargs)
        return reshape.broadcast(self_or_cls._obj, *others, **kwargs)

    def broadcast_to(self, other, **kwargs):
        if isinstance(other, Base_Accessor):
            other = other._obj
        return reshape.broadcast_to(self._obj, other, **kwargs)

    def make_symmetric(self):
        return reshape.make_symmetric(self._obj)

    def unstack_to_array(self, **kwargs):
        return reshape.unstack_to_array(self._obj, **kwargs)

    def unstack_to_df(self, **kwargs):
        return reshape.unstack_to_df(self._obj, **kwargs)
    ...
    def concat(self_or_cls, *others, as_columns=None, broadcast_kwargs={}):
        ...
        broadcasted = reshape.broadcast(*objs, **broadcast_kwargs)
        broadcasted = tuple(map(reshape.to_2d, broadcasted))
        if is_pandas(broadcasted[0]):
            concated = pd.concat(broadcasted, axis=1)
            if as_columns is not None:
                concated.columns = indexing.combine(as_columns, broadcasted[0].columns)
        else:
    ...
    def apply_and_concat(self, ntimes, *args, apply_func=None, as_columns=None, **kwargs):
        ...
        if is_numba_func(apply_func):
            ...
            result = combine.apply_and_concat_nb(np.asarray(self._obj), ntimes, apply_func, *args, **kwargs)
        else:
            result = combine.apply_and_concat(np.asarray(self._obj), ntimes, apply_func, *args, **kwargs)
        # Build column hierarchy
        if as_columns is not None:
            new_columns = indexes.combine(as_columns, reshape.to_2d(self._obj).columns)
        else:
            new_columns = indexes.tile(reshape.to_2d(self._obj).columns, ntimes)
    ...
    def combine_with(self, other, *args, combine_func=None, broadcast_kwargs={}, **kwargs):
        ...
        new_obj, new_other = reshape.broadcast(self._obj, other, **broadcast_kwargs)
    ...
    def combine_with_multiple(self, others, *args, combine_func=None, concat=False,
        ...
        new_obj, *new_others = reshape.broadcast(self._obj, *others, **broadcast_kwargs)
        ...
        if concat:
            ...
            if is_numba_func(combine_func):
                ...
                result = combine.combine_and_concat_nb(broadcasted[0], broadcasted[1:], combine_func, *args, **kwargs)
            else:
                result = combine.combine_and_concat(broadcasted[0], broadcasted[1:], combine_func, *args, **kwargs)
            if as_columns is not None:
                new_columns = indexes.combine(as_columns, reshape.to_2d(new_obj).columns)
            else:
                new_columns = indexes.tile(reshape.to_2d(new_obj).columns, len(others))
            ...
            if is_numba_func(combine_func):
                ...
                result = combine.combine_multiple_nb(broadcasted, combine_func, *args, **kwargs)
            else:
                result = combine.combine_multiple(broadcasted, combine_func, *args, **kwargs)
