...
def ma_caching_nb(ts, windows, ewms):
    ...
    for i in range(windows.shape[0]):
        ...
        if h not in cache_dict:
            if ewms[i]:
                ma = generic.nb.ewm_mean_nb(ts, windows[i])
            else:
                ma = generic.nb.rolling_mean_nb(ts, windows[i])
            cache_dict[h] = ma
    return cache_dict
...
def mstd_caching_nb(ts, windows, ewms):
    ...
    for i in range(windows.shape[0]):
        ...
        if h not in cache_dict:
            if ewms[i]:
                mstd = generic.nb.ewm_std_nb(ts, windows[i])
            else:
                mstd = generic.nb.rolling_std_nb(ts, windows[i])
            cache_dict[h] = mstd
    return cache_dict
...
def rsi_caching_nb(ts, windows, ewms):
    ...
    delta = generic.nb.diff_nb(ts)[1:, :]  # otherwise ewma will be all NaN
    up, down = delta.copy(), delta.copy()
    up = generic.nb.set_by_mask_nb(up, up < 0, 0)
    down = np.abs(generic.nb.set_by_mask_nb(down, down > 0, 0))
    # Cache
    cache_dict = dict()
    for i in range(windows.shape[0]):
        h = hash((windows[i], ewms[i]))
        if h not in cache_dict:
            if ewms[i]:
                roll_up = generic.nb.ewm_mean_nb(up, windows[i])
                roll_down = generic.nb.ewm_mean_nb(down, windows[i])
            else:
                roll_up = generic.nb.rolling_mean_nb(up, windows[i])
                roll_down = generic.nb.rolling_mean_nb(down, windows[i])
            roll_up = generic.nb.prepend_nb(roll_up, 1, np.nan)  # bring to old shape
            roll_down = generic.nb.prepend_nb(roll_down, 1, np.nan)
...
def stoch_caching_nb(close_ts, high_ts, low_ts, k_windows, d_windows, d_ewms):
    ...
    for i in range(k_windows.shape[0]):
        h = hash(k_windows[i])
        if h not in cache_dict:
            roll_min = generic.nb.rolling_min_nb(low_ts, k_windows[i])
            roll_max = generic.nb.rolling_max_nb(high_ts, k_windows[i])
...
def stoch_apply_nb(close_ts, high_ts, low_ts, k_window, d_window, d_ewm, cache_dict):
    ...
    if d_ewm:
        percent_d = generic.nb.ewm_mean_nb(percent_k, d_window)
    else:
        percent_d = generic.nb.rolling_mean_nb(percent_k, d_window)
...
def macd_apply_nb(ts, fast_window, slow_window, signal_window, macd_ewm, signal_ewm, cache_dict):
    ...
    if signal_ewm:
        signal_ts = generic.nb.ewm_mean_nb(macd_ts, signal_window)
    else:
        signal_ts = generic.nb.rolling_mean_nb(macd_ts, signal_window)
...
def atr_caching_nb(close_ts, high_ts, low_ts, windows, ewms):
    ...
    tr0 = high_ts - low_ts
    tr1 = np.abs(high_ts - generic.nb.fshift_nb(close_ts, 1))
    tr2 = np.abs(low_ts - generic.nb.fshift_nb(close_ts, 1))
    ...
    for i in range(windows.shape[0]):
        ...
        if h not in cache_dict:
            if ewms[i]:
                atr = generic.nb.ewm_mean_nb(tr, windows[i])
            else:
                atr = generic.nb.rolling_mean_nb(tr, windows[i])
            cache_dict[h] = atr
